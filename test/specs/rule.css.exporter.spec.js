...
describe( 'Export Rules CSS', () =>
{
    ...
    it( 'if the shorthand option is passed, should export the first value of every property in a rule in shorthand order', () =>
    {
        ...
        expect( exportRulesCss( options, testState7 ) ).to.eql( [ '1s ease 1ms 1 normal forwards running schinken', '15ms ease 1ms 1 normal forwards running eier' ] );
        expect( exportRulesCss( options, testState8 ) ).to.eql( [ 'ease bretzel' ] );
        expect( exportRulesCss( options, testState9, rules ) ).to.eql( ['0s none obst'] );
    } );
